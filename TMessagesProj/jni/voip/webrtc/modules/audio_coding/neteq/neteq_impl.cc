  ...
  *muted = false;
  ...
  switch (operation) {
    ...
    case Operation::kFastAccelerate: {
      ...
      int yasu_max_accelerate_passes = 1;

      if (fast_accelerate) {
        const size_t yasu_ms = fs_hz_ / 1000;
        // YASU: NetEq delay accounting.
        // Do not count AlgorithmBuffer as an independent backlog stage.
        // The actual future playout delay is PacketBuffer + SyncBuffer.
      ...
      return_value = DoAccelerate(
          decoded_buffer_.get(), length, speech_type, play_dtmf,
          fast_accelerate, yasu_max_accelerate_passes);
      break;
    }
